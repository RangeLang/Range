...
extension Parser {
    mutating func parseDeclaration() throws -> DeclarationNode {
        ...
        while true {
            ...
            case .atAttribute(let attr, _) where attr == "StyleModifier":
                objects.append(.styleModifier(try parseStyleModifierDeclaration()))
            default:
                break
            }
    ...
    mutating func parseStyleModifierDeclaration() throws -> StyleModifierDeclaration {
        try consume(.atAttribute(name: "StyleModifier", argument: nil))
        let name = try consumeIdentifier()
        var parameters: [StyleModifierParameter] = []
        if peek() == .colon {
            throw ParseError(
        ...
        if peek() == .leftBrace {
            ...
            while peek() != .rightBrace {
                if case .keyword(NeatSyntax.Keyword.variable.rawValue) = peek() {
                    advance()
                    let parameterName = try consumeIdentifier()
                    try consume(.colon)
                    let typeName = try consumeTypeReference()
                    parameters.append(
                        StyleModifierParameter(name: parameterName, typeName: typeName)
                    )
                    continue
                }
                advance()
        ...
        return StyleModifierDeclaration(
            name: name,

...
func fetchUsername(id: Int) -> Promise<String, FetchError> {
...
func useLiveRequest(request: Promise<String, FetchError>) {
    Logger.info("live request typed")
}

func useSettledResult(result: Result<String, FetchError>) {
...
struct NeatMain {
    static func main() throws {
        var liveRequest: Promise<String, FetchError> = fetchUsername(id: 1)
        useLiveRequest(request: liveRequest)
        Logger.info("request started")
        let settledResult: Result<String, FetchError> = settle(fetchUsername(id: 1))

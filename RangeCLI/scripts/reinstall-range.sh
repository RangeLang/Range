...
INSTALL_SCRIPT="${SCRIPT_DIR}/install-range.sh"
TARGET_INPUT="${1:-}"
EXTRA_FLAG="${2:-}"
AUTO_YES=0
...
if [[ -z "${TARGET_INPUT}" ]]; then
  confirm_wipe "installed range binaries" || exit 1
...
if [[ "${PROJECT_DIR}" == */range ]]; then
  PROJECT_DIR="${PROJECT_DIR%/range}"
fi
...
"${INSTALL_SCRIPT}"
"${SCRIPT_DIR}/../.build/release/RangeCLI" create "${PROJECT_DIR}"

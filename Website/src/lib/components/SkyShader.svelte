<script lang="ts">
  ...
  // Prop-driven transitions invalidate immediately; the shader's own ambient
  // clock keeps the visible field alive unless reduced motion requests a still.
  $effect(() => {
    void concreteness;
    void compact;
    void fisheyeAmount;
  ...
  const fragmentSource = `
    ...
    uniform float u_orbit_duration;
      ...
      if (fieldVisible > 0.5) {
        ...
        vec2 underPoint = fieldPoint
          + (fisheyePoint - spherePoint)
            ...
            * fisheyeScale
            * lensVisible;
        // Trace a slow circle through the field, as though the pointer were
        // orbiting the viewport center. Direction changes continuously without
        // accumulating a one-way translation.
        float directionAngle = u_time * (
          6.2831853 / max(u_orbit_duration, 0.001)
        );
        vec2 orbitDirection = vec2(cos(directionAngle), sin(directionAngle));
        vec2 directedPoint = underPoint + orbitDirection * 0.028;
        float grain = valueNoise(directedPoint * 4.5);
        ...
        float coarsePhase = hash12(
          floor(directedPoint * 16.8) + vec2(7.3, 2.1)
        ) * 6.2831;
        float finePhase = hash12(
          floor(directedPoint * 56.0) + vec2(19.7, 4.6)
        ) * 6.2831;
        float coarseWave = 0.5 + 0.5 * sin(u_time * 1.65 + coarsePhase);
        float fineWave = 0.5 + 0.5 * sin(u_time * 2.35 + finePhase);
        float coarseSpark = pow(coarseWave, 3.0);
        float fineSpark = pow(fineWave, 5.0);
        float twinkle = clamp(u_twinkle_amount, 0.0, 1.0);
        float coarseShimmer = mix(
          1.0,
          0.32 + 1.18 * coarseSpark,
          twinkle
        );
        float fineShimmer = mix(
          1.0,
          0.38 + 1.35 * fineSpark,
          twinkle
        );
  ...
  onMount(() => {
    ...
    const orbitDurationLocation = context.getUniformLocation(
      program,
      "u_orbit_duration",
    );
    ...
    let frame = 0;
    let visible = true;
    // Ambient sky behavior belongs here rather than in each consumer. Callers
    // can fade the stars as part of a transition, but their density, shimmer,
    // drift, and clock remain one shared visual language.
    const startedAt = performance.now();
    const glitterIntensity = 1.15;
    const twinkleIntensity = 1;
    let density = 1;
    let viewportWidth = 1;
    ...
    const draw = (now: number) => {
      ...
      context.uniform1f(orbitDurationLocation, Math.max(orbitDuration, 0.001));
      ...
      context.uniform1f(
        glitterLocation,
        glitterIntensity * Math.min(1, Math.max(0, starOpacity)),
      );
      ...
      context.uniform1f(twinkleLocation, twinkleIntensity);

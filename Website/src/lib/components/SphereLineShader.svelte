<script lang="ts">
  ...
  const fragmentSource = `
    ...
    void main() {
      ...
      float thirdGlow = glow(point, third, 0.09);
      float cloud = firstGlow * 0.42 + secondGlow * 0.38 + thirdGlow * 0.3;
      cloud += (hash(floor(gl_FragCoord.xy * 0.45)) - 0.5) * 0.028;

      vec3 color = mix(
        oklch(0.965, 0.018, 252.0),
      ...
      float goldBloom =
        ...
        + exp(-distance(point, third + vec2(-0.018, 0.025)) * 22.0);
      float slateBloom =
        exp(-distance(point, second + vec2(-0.025, 0.035)) * 18.0);
      color += oklch(0.97, 0.055, 84.0) * goldBloom * 0.24;
      color += oklch(0.95, 0.025, 252.0) * slateBloom * 0.2;

      float lineField =
        rays(point, first, 0.16, 19.0, time * 0.12)
        + rays(point, second, 0.12, 15.0, 1.7 - time * 0.1)
        + rays(point, third, 0.09, 13.0, 3.1 + time * 0.08);
      vec3 lineColor = mix(
        oklch(0.88, 0.055, 252.0),
        oklch(0.9, 0.11, 84.0),
        paletteVariation
      );
      color = mix(color, lineColor, min(lineField * 0.44, 0.52));
      ...
      vec4 thirdSphere = renderedSphere(
        point,
        third,
        0.09,
        0.92,
      ...
      color = mix(color, thirdSphere.rgb, thirdSphere.a);

...
test("keeps the benchmark artifact complete and versioned", async () => {
  ...
  assert.match(rangeScaleElement, /pixelRatio = Math\.min\(8/);
  assert.match(rangeScaleElement, /this\.#context\.setTransform\(1, 0, 0, 1, 0, 0\)/);
  assert.match(rangeScaleElement, /const markWidth = Math\.max\(1, Math\.round\(mark\.measure \* pixelRatio\)\)/);
  assert.match(rangeScaleElement, /const markHeight = Math\.max\(1, Math\.round\(Math\.min\(1, mark\.stroke\) \* pixelRatio\)\)/);
  ...
  assert.doesNotMatch(rangeScaleElement, /shadowBlur/);

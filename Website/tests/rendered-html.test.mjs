...
test("renders the Range landing page", async () => {
  ...
  assert.match(html, /src="\/range-optical-guide\.js\?guide=glyph-contact-v4"/);
...
test("merges linear scale and pinch marks deterministically", async () => {
  ...
  assert.ok(marks.every((mark) => mark.blur >= 0 && mark.blur <= 1.6));
  ...
  assert.ok(marks[7].opacity < 0.1);
  assert.ok(marks[7].blur > 1.3);
...
test("keeps the benchmark artifact complete and versioned", async () => {
  ...
  assert.match(styles, /\.landingWordmark \.rangeWord,\s*\.routeWordmark \.rangeWord\s*{[^}]*view-transition-name:\s*range-home-wordmark/s);
  assert.match(styles, /\.landingHero \.rangeTitleWord,\s*\.pageHeader h1 > span:first-child\s*{[^}]*view-transition-name:\s*range-performance-wordmark/s);
  ...
  assert.match(renderer, /class="landingSupport"><p>.*abstraction<\/p><div class="landingActions">/s);
  assert.match(styles, /\.landingSupport\s*{[^}]*width:\s*fit-content[^}]*margin:\s*40px 0 0 var\(--range-title-leading\)/s);
  assert.match(styles, /\.landingHero p\s*{[^}]*margin:\s*0/s);
  assert.match(styles, /\.landingActions\s*{[^}]*justify-content:\s*flex-end[^}]*margin:\s*32px 0 0/s);
  ...
  assert.match(styles, /\.landingActions\s*{[^}]*--range-actions-end-shift/s);
  assert.match(styles, /view-transition-group\(range-home-wordmark\)/);
  ...
  assert.match(styles, /range-typed-text\s*{[^}]*view-transition-name:\s*range-performance-suffix/s);
  assert.match(styles, /range-typed-text\[data-route-pending\]::after\s*{[^}]*content:\s*none/s);
  assert.match(styles, /range-typed-text\[data-collapsing\]\s*{[^}]*margin-left:\s*-0\.24em[^}]*padding-left:\s*0\.24em[^}]*overflow:\s*clip/s);
  assert.match(styles, /range-typed-text\[data-collapsing\] > \[data-collapse-content\]\s*{[^}]*range-performance-collapse 180ms cubic-bezier\(0\.32, 0\.72, 0, 1\)/s);
  ...
  assert.match(styles, /range-route-performance\.range-route-forward::view-transition-old\(range-performance-wordmark\)[^{]*\{[^}]*opacity:\s*1/s);
  assert.match(styles, /range-route-performance\.range-route-forward::view-transition-new\(range-performance-wordmark\)[^{]*\{[^}]*opacity:\s*0/s);
  assert.match(styles, /range-route-performance\.range-route-backward::view-transition-new\(range-performance-wordmark\)[^{]*\{[^}]*opacity:\s*1/s);
  assert.match(styles, /range-route-performance\.range-route-backward::view-transition-old\(range-performance-wordmark\)[^{]*\{[^}]*opacity:\s*0/s);
  ...
  assert.match(rangeOpticalGuide, /document\.createRange\(\)/);
  assert.match(rangeOpticalGuide, /#inkEnd\(github, this\.#shifts\.actionEnd\)/);
  assert.match(rangeOpticalGuide, /--range-actions-end-shift/);
  ...
  assert.match(styles, /view-transition-name:\s*range-zero-index/);
  ...
  assert.match(styles, /\.landingHero \[data-scale-end\] > span\s*{[^}]*left:\s*50%[^}]*transform:\s*translate\(calc\(-50% \+ var\(--range-one-inline-shift, 0px\)\), var\(--range-one-contact-shift, 0px\)\)/s);
...
test("uses Web Components without React, Next, or Vinext", async () => {
  const [packageText, worker, renderer, components, layoutReady, navigation, typedText] = await Promise.all([
  ...
  assert.match(navigation, /let activeRoute = location\.href/);
  assert.match(navigation, /activeRoute = destination/);
  assert.match(navigation, /`range-route-\$\{direction\}`/);
  ...
  assert.match(worker, /range-ui-v96\.css/);
  assert.match(worker, /replace\('\/range-ui\.css', '\/range-ui-v96\.css'\)/);
...
test("serves Geist Sans and Mono from the site artifact", async () => {
  ...
  const response = await render("/");
  assert.match(await response.text(), /href="\/range-ui-v96\.css"/);
});

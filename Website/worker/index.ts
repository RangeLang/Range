...
const worker = {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/globals.css" || url.pathname === "/range-geist.css" || url.pathname === "/range-ui.css" || url.pathname === "/range-ui-v85.css") {
    ...
    if (rendered) {
      const html = rendered.html
        .replace('/range-ui.css', '/range-ui-v85.css')
        .replace('/range-typed-text.js', '/range-typed-text.js?version=84')
        .replace('/range-navigation-v2.js', '/range-navigation-v2.js?version=85');

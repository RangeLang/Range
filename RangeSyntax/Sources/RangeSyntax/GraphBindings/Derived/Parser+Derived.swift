...
extension Parser {
    ...
    func collectBuilderHooks(from statements: [Statement], into hooks: inout Set<String>) {
        for statement in statements {
            ...
            case .background(let background):
                collectBuilderHooks(from: background.body, into: &hooks)
            case .deferBlock(let deferred):
                collectBuilderHooks(from: deferred.body, into: &hooks)
    ...
    func isDerivedDeclarationStart() -> Bool {
        ...
        let offset = isMacroApplicationStart() ? macroApplicationLookaheadLength() : 0
        guard peek(offset: offset) == .keyword(RangeSyntax.Keyword.derived.rawValue) else {
            return false
        }

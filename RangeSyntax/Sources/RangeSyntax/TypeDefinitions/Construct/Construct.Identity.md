...
- Foundational plain-value types such as `Int` belong to ` construct`, not this identity model.
- Compiler structural constructs such as `Closure` and `Block` may also belong to ` construct` when they are non-identity-bearing values.

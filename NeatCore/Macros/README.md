...
- Macro parameters can use `capture T` to request call-site syntax capture for
  ...
  capture.
- Parameter-targeted macros are bootstrap-executed from Neat macro bodies through declaration/application facets such as `target.declaration.type.rewrite(...)` and `target.application.arguments.rewrite(...)`.

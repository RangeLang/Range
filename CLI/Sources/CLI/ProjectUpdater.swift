...
struct ProjectUpdater {
    ...
    static func updateInstalledCLI(
        ...
        if version == "latest" {
            ...
            guard status.updateAvailable else {
                TerminalLog.out("CLI is already up to date.", level: .success)
                return
            }
        }
        ...
        TerminalLog.out("Updated CLI.", level: .success)
    }
    ...
    func run() throws {
        ...
        if updateCLI {
            try updateCLIIfAvailable(from: root)
        }
    ...
    private func updateCLIIfAvailable(from root: URL) throws {
        let script = root.appendingPathComponent("scripts/install-range.sh", isDirectory: false)
        if !FileManager.default.fileExists(atPath: script.path) {
            TerminalLog.out(
                "Skipping CLI update: scripts/install-range.sh not found in \(root.path)",
                level: .warning
            )
            return
        }

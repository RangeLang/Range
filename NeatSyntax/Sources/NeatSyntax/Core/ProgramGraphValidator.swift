...
public struct ProgramGraphValidator {
    ...
    public func validate(_ graph: ProgramGraph) throws {
        let entityIDs = Set(graph.semanticGraph.entities.map(\.id))

        guard entityIDs.count == graph.semanticGraph.entities.count else {
            throw SemanticValidationError("ProgramGraph contains duplicate entity IDs.")
        }

        for relation in graph.semanticGraph.relations {
